...
pub fn get_locks(mid_key: &str) -> (String, String, String, String) {
    let net_lock = IdBuilder::new(Encryption::SHA256)
        .add_component(HWIDComponent::MacAddress)
        .build(mid_key)
        .expect("To generate a string");

    let storage_lock = IdBuilder::new(Encryption::SHA256)
        .add_component(HWIDComponent::DriveSerial)
        .build(mid_key)
        .expect("To generate a string");

    let cpu_lock = IdBuilder::new(Encryption::SHA256)
...
impl SysInfo {
    ...
    #[must_use]
    pub(crate) fn decode_from_string(input: &str, public_key: &str) -> Option<Self> {
        ...
        rmp_serde::from_read::<&[u8], Self>(&*decrypted).ok()
    }
}
